...
function findShortestPath(from: {x: number, y: number}, to: {x: number, y: number}, distMap: number[][]): number {
...
const distMap = Array.from({length: height}, () => Array(height).fill(Infinity));
...
for (let y = 0; y < height; y++) {
  for (let x = 0; x < width; x++) {
    [
      ...
      if (cheatX < 0 || cheatX >= width ||cheatY < 0 || cheatY >= height) return;
      if (map[cheatY][cheatX] === "#") return;

      const beforeDist = distMap[y][x]
      const afterDist = distMap[cheatY][cheatX]

      const diff = afterDist - beforeDist - 2; // 2 because the cheat takes 2
      if (diff < 100) return;
      total++;
    })
  }
